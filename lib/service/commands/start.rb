    ...
    class Start < Command
      def run
        ...
        if service.running?
          ...
          puts "Starting '#{Paint[service.name, :cyan]}' service:\n\n"
          status_text = Paint["Starting service", '#2794d8']
          print "   > "
          begin
            Whirly.start(
              spinner: 'star',
          ...
          if success
            puts "The '#{Paint[service.name, :cyan]}' service has been started."
          else
            raise ServiceOperationError, "unable to start service"
          end
        end
      end
